...
download_rx_error(gpointer o, const char *reason, ...)
{
	struct download *d = o;
	char msg[1024];
	va_list args;
 ...
 */
static void
download_unqueue(struct download *d, gboolean removing)
{
	download_check(d);
	g_assert(DOWNLOAD_IS_QUEUED(d));
 ...
 */
static void
download_queue_delay(struct download *d, guint32 delay, const char *fmt, ...)
{
	va_list args;
 ...
 */
static void
download_queue_hold(struct download *d, guint32 hold, const char *fmt, ...)
{
	va_list args;
...
download_handle_thex_uri_header(struct download *d, header_t *header)
{
	...
	if ('/' != uri_start[0]) {
		if (GNET_PROPERTY(tigertree_debug)) {
			g_message("TTH X-Thex-URI header has no valid URI (%s): \"%s\"",
	...
	if (endptr) {
		...
		if (strlen(urn) < TTH_BASE32_SIZE) {
			if (GNET_PROPERTY(tigertree_debug)) {
				g_message("TTH X-Thex-URI header has no root hash "
					"for %s from %s",
					download_basename(d), download_host_info(d));
			}
			return;
		...
		if (GNET_PROPERTY(tigertree_debug)) {
			g_message("TTH X-Thex-URI header has no root hash (%s): \"%s\"",
				download_host_info(d), uri_start);
		}
		/*
...
handle_content_urn(struct download *d, header_t *header)
{
	...
	if (found_sha1) {
		if (d->file_info->tth) {
			...
			if (GNET_PROPERTY(tigertree_debug)) {
				g_message("TTH discovered root hash (%s) for %s from %s",
					tth_base32(&tth),
					download_basename(d), download_host_info(d));
			}
			file_info_got_tth(d->file_info, &tth);
 ...
 */
void
download_move_done(struct download *d, const char *pathname, guint elapsed)
{
	fileinfo_t *fi;
	...
	const struct tth *leaves, size_t num_leaves)
{
	...
	if (GNET_PROPERTY(tigertree_debug)) {
		g_message("TTH tree sweep: file=\"%s\", filesize=%s, slice size=%s",
			download_basename(d),
			filesize_to_string(download_filesize(d)),
	...
	if (bad_slices > 0) {
		...
		if (GNET_PROPERTY(tigertree_debug)) {
			g_message("TTH tree sweep: all %lu slice%s okay",
...
download_thex_done(struct download *d)
{
	...
	if (!thex_download_finished(d->thex)) {
		if (GNET_PROPERTY(tigertree_debug)) {
			g_message("TTH discarding tigertree data from %s: Bad THEX data",
				download_host_info(d));
		}
		goto finish;
	}
	...
	if (NULL == fi) {
		if (GNET_PROPERTY(tigertree_debug)) {
			g_message("TTH discarding tigertree data from %s: No more download",
				download_host_info(d));	
		}
		cancel_all = TRUE;
		goto finish;
