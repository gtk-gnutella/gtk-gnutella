...
/**
 * Check whether slist contains the `key' whereas equalness is determined
 * using `func'.
 */
gboolean
slist_contains(const slist_t *slist, gconstpointer key, GCompareFunc func,
	gpointer *orig_key)
{
	GSList *item;
	...
	item = g_slist_find_custom(slist->head, key, func);
	if (item) {
		if (orig_key) {
			*orig_key = item->data;
		}
