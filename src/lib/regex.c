/*
 * Inclusion of this file in gkt-gnutella is:
 ...
 * @file
 *
 * Regular expression support: regcomp(), regexec(), regfree().
 *
...
#define TRANSLATE(d) (translate ? (translate[(unsigned char) (d)]) : (char) (d))
...
#define PUSH_FAILURE_POINT(pattern_place, string_place, failure_code)	\
  do {									\
    ...
    for (this_reg = lowest_active_reg; \
		 UNSIGNED(this_reg) <= highest_active_reg;	\
         this_reg++)							\
      {									\
...
size_t
regerror (errcode, unused_preg, errbuf, errbuf_size)
    int errcode;
    const regex_t *unused_preg;
    char *errbuf;
    size_t errbuf_size;
{
  const char *msg;
