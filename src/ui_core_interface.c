 ...
 *	
 * Interface between the ui and the core
 *
 *----------------------------------------------------------------------
...
/*	UI/CORE Interface File
 *
 *	This file is divided into 3 parts:
 *		1 - Includes needed by the interface to allow gui/core communication
 *		2 - Functions the CORE uses to access the UI
 *		3 - Functions the UI uses to access the CORE	
 *	
...
#include "downloads_gui_common.h" /* for popup_queue */
#include "gtk-missing.h" /* for gtk_main_flush */
#include "gui.h" /* for gui_update_files_scanned */
...
/*
 *	SECTION 2 - Functions the CORE uses to access the UI
 */
	...
	guint32 max_downloads)
{
	...
	gtk_widget_set_sensitive(lookup_widget
		(popup_queue, "popup_queue_start_now"), 
		(running_downloads < max_downloads));	
#else
...
{
	return search_gui_new_search(query, flags, search);
}
...
/*	
 *	SECTION 3 - Functions the UI uses to access the CORE
 */
