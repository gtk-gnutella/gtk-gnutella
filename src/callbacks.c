...
void on_button_host_catcher_connect_clicked(GtkButton * button,
											gpointer user_data)
{
	if (GTK_CLIST(clist_host_catcher)->selection) {
		...
		while (l) {
			h = (struct gnutella_host *)
				gtk_clist_get_row_data(GTK_CLIST(clist_host_catcher),
									   (gint) l->data);
			l = l->next;
...
void on_clist_host_catcher_select_row(GtkCList * clist, gint row,
									  ...
									  gpointer user_data)
{
	...
	h = (struct gnutella_host *)
		gtk_clist_get_row_data(GTK_CLIST(clist_host_catcher), row);

	gtk_entry_set_text(GTK_ENTRY(entry_host),
					   ip_port_to_gchar(h->ip, h->port));
}

void on_clist_host_catcher_unselect_row(GtkCList * clist, gint row,
										gint column, GdkEvent * event,
										gpointer user_data)
{
...
gboolean on_clist_host_catcher_button_press_event(GtkWidget * widget,
												  GdkEventButton * event,
												  gpointer user_data)
{
	gint row, col;
	...
	if (event->button == 3) {
		gtk_widget_set_sensitive(popup_hosts_export,
								 (gboolean) sl_catched_hosts);
		gtk_clist_unselect_all(GTK_CLIST(clist_host_catcher));
		gtk_menu_popup(GTK_MENU(popup_hosts), NULL, NULL, NULL, NULL, 3,
					   0);
		return TRUE;
	}
	...
	if (!gtk_clist_get_selection_info
		(GTK_CLIST(clist_host_catcher), event->x, event->y, &row, &col))
		return FALSE;
